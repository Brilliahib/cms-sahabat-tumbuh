...
import { useGetAllArticleIndication } from "@/http/article/get-article-indication";
...
export default function ArticleIndicationList() {
  const { data: session, status } = useSession();
  const { data, isPending } = useGetAllArticleIndication(
      ...
      <div className="grid md:grid-cols-3 grid-cols-1 gap-8">
        {isPending
          ...
          : filteredArticles?.map((article) => (
              <Link
                key={article.id}
                href={`/dashboard/tracking/indication/${article.id}`}
              >
